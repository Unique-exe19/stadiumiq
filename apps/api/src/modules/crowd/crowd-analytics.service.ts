      ...
      async () => {
        ...
        const avgDensity =
          recentReadings.length > 0
            ? recentReadings.reduce((sum, r) => sum + r.densityPerSqMeter, 0) /
        ...
        const trend =
          recentReadings.length >= 2
            ? recentReadings[0]!.densityPerSqMeter -
              recentReadings[recentReadings.length - 1]!.densityPerSqMeter
            : 0;
        ...
        return {
          ...
          riskFactors:
            predictedLevel === 'critical' || predictedLevel === 'high'
              ? ['Increasing density trend', 'Post-halftime crowd surge expected']
              : [],
          recommendedInterventions:
            predictedLevel === 'critical'
              ? [
                  'Open Gate C overflow',
                  'Deploy 3 additional stewards',
                  'Activate PA system guidance',
                ]
              : [],
        };
      },
