  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <defs>
          {ZONES.map((_z, i) => (
            <radialGradient key={i} id={`hg-${i}`} cx="50%" cy="50%" r="50%">
              <stop
                offset="0%"
                stopColor={heatColor(intensities[i] ?? 0)}
                stopOpacity={(intensities[i] ?? 0) * 0.75}
              />
        ...
        {/* Gate labels */}
        <text x="195" y="14" textAnchor="middle" fontSize="9" fill="#64748b" fontWeight="600">
          Gate A
        </text>
        <text x="18" y="113" textAnchor="middle" fontSize="9" fill="#64748b" fontWeight="600">
          Gate B
        </text>
        <text x="372" y="113" textAnchor="middle" fontSize="9" fill="#64748b" fontWeight="600">
          Gate C
        </text>
        <text x="195" y="210" textAnchor="middle" fontSize="9" fill="#64748b" fontWeight="600">
          Gate D
        </text>

        {/* Your seat marker */}
        <circle cx="255" cy="82" r="6" fill="#1d4ed8" stroke="white" strokeWidth="1.5" />
        <text x="255" y="86" textAnchor="middle" fontSize="5.5" fontWeight="bold" fill="white">
          C42
        </text>
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <main className="pt-20 pb-16 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto" id="main-content">
        ...
        <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-2xl p-5 mb-6 text-white flex items-center justify-between shadow-card">
          <div>
            <div className="text-xs font-medium text-primary-200 mb-1 uppercase tracking-wide">
              Match Day · Group Stage A
            </div>
        ...
        <section aria-labelledby="quick-actions-heading" className="mb-8">
          <h2 id="quick-actions-heading" className="text-base font-semibold text-slate-700 mb-3">
            Quick Actions
          </h2>
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {QUICK_ACTIONS.map(({ icon: Icon, label, color, bg, href }) => (
              ...
              >
                <div
                  className={`w-10 h-10 ${bg} rounded-xl flex items-center justify-center group-hover:scale-105 transition-transform`}
                >
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-soft p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <Link
                  href="#"
                  className="ml-2 text-xs text-primary-600 font-medium hover:underline flex items-center gap-1"
                >
            ...
            <ul className="space-y-2.5" role="list">
              {GATE_STATUS.map((g) => (
                ...
                >
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-semibold ${crowdColor[g.crowd]}`}
                  >
                    {g.status === 'Closed'
                      ? 'Closed'
                      : g.crowd.charAt(0).toUpperCase() + g.crowd.slice(1)}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="flex-1 bg-white rounded-2xl border border-slate-100 shadow-soft p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">
                Your Seat: Section 12 · Row C · Seat 42
              </p>
              <p className="text-xs text-slate-500 mt-0.5">
                Enter Gate C → Elevator 2 → Level 3 → Follow signs
              </p>
            </div>
            <Link
              href="#"
              className="ml-auto px-3 py-1.5 text-xs font-semibold text-primary-600 bg-primary-50 hover:bg-primary-100 rounded-lg transition-colors whitespace-nowrap"
            >
              Navigate
            </Link>
          </div>
