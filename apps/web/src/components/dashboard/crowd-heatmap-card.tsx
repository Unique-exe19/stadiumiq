...
export function CrowdHeatmapCard() {
  return (
    <section className="glass-card rounded-2xl p-5 h-full" aria-labelledby="heatmap-heading">
      ...
      <div className="relative" aria-label="Stadium crowd density heatmap" role="img">
        <svg viewBox="0 0 100 100" className="w-full aspect-square" aria-hidden="true">
          {/* Stadium outline */}
          <ellipse
            cx="50"
            cy="50"
          ...
          {ZONES.filter((z) => z.density > 0).map((zone) => (
            <g key={zone.id}>
              ...
              />
              <circle cx={zone.x} cy={zone.y} r={3} fill="white" opacity={0.8} />
            </g>
          ))}
